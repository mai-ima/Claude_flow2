...
| `feedback_reply.py` | ご意見への返信、書きかけの保持、内部メモが漏れないこと |
| `admin_logs.py` | 運用の記録の絞り込みと削除、監査ログだけ扱いが違うこと |
| `notify_reset.py` | お知らせの削除と、初期状態に戻したとき案内が戻ること |
| `timezone.py` | 日付と時刻が日本時間で扱われること（TZ=UTC で起動して確認） |
