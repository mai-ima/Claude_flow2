...
## 利用者に見える文言（全画面で統一）
金融まわりを預かるサービスとして、話し言葉を混ぜない。くだけた語が1つ
混ざるだけで、作りの粗いサービスに見える。

- **敬体で書く**。利用者の行為には尊敬語・謙譲語を添える。
  - 「返信がほしい」→「返信を希望する」
  - 「読めます」「見られます」→「ご覧いただけます」「ご確認いただけます」
  - 「送れます」→「送信いただけます」／「使えます」→「ご利用いただけます」
- **話し言葉を使わない**。「サッと」「ざっくり」「いろいろ」「ちゃんと」
  「〜だけです」→「すばやく」「おおまかに」「さまざま」「正しく」「〜のみです」。
- **操作の言い方を揃える**。押す→選択する、消す→削除する、
  やめる→キャンセル（確定した語なのでカタカナのまま使う）。
  「取り消す」は正式な語なのでそのまま用いてよい。
- **専門用語は避ける**。それでも要る場合は、その場で言い換えを添える。
- **断定を避けない**。「〜かもしれません」で濁さず、条件を書いて言い切る。
- コード内のコメントはこの限りではない（開発者向けなので簡潔さを優先）。
